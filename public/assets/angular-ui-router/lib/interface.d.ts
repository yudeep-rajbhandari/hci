 ...
 *     }
 *   },
 *   templates: "<ul><li ng-repeat='folder in allfolders'>{{folder.name}}</li></ul>",
 ...
 * `templates` (or `templateProvider`) and a `controller` (or `controllerProvider`).
 *
...
export interface Ng1Controller {
     ...
     *   templates: '<input ng-model="$ctrl.data" type="text">',
     *   bindings: { 'data': '<' },
     *   controller: function() {
     *
...
/**
 * Manages which templates-loading mechanism to use.
 *
